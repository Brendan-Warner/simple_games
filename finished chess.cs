...
public class king:peice
{
    ...
    public override void output_moves()
    {
        ...
        Console.WriteLine("7.Move diagnolly to the down and right.");
        Console.WriteLine("8.move diagnolly to the up and left.");
        Console.WriteLine("25. castle with the rook on the right side of the board.");
	Console.WriteLine("26. castle with the rook on the left side of the board.");
    }
}
...
class HelloWorld
{
    ...
    static void test_board_draw()//update this for testing checkmate. then for putting in to check.
    {
        ...
        for(i = 1; i < 9; i++)
        {
            if(i==2 && j == 3 || i==2 && j == 2 || i == 4 && j == 1)
            {
                board.Add(create_castle(id, "w", j, i));
                id++;
            }
            else if(i == 1 && j== 7)
            {
               board.Add(create_castle(4, "w", j, i)); 
            }
    ...
    static bool put_in_check(List<peice> p, int move, string color)
    {
	    ...
	    int previous_row = board[place].row;
	    int previous_column = board[place].column;
	    int next_row = board[place+update].row;
	    int next_column = board[place+update].column;
	    ...
	    board[place+update].row = next_row;
	    board[place+update].column = next_column;
	    board[place].row = previous_row;
	    board[place].column = previous_column;
    ...
    static bool still_in_check(List<peice> p, int place1, int place2, string color)
    {
	    ...
	    int previous_row = board[place1].row;
	    int previous_column = board[place1].column;
	    int next_row = board[place2].row;
	    int next_column = board[place2].column;
	    tempHolder[0] = board[place2];
	    board[place2] = board[place1];
	    board[place1] = tempEmpty;
	
	
	    board[place2].row = next_row;
	    board[place2].column = next_column;
	    board[place1].row = previous_row;
	    board[place1].column = previous_column;
    ...
    static bool check_can_move(List<peice> p, int move, string color)//checks if the peice can move in a givin direction for a move, if not, then it returns false, else, the peice can move.
    {
	    ...
	    if(move == 25 && board[place + 1].type == " " && board[place +2].type == " " && board[place +3].type == "c"&& board[place +3].color == color && inCheck != color)
	    {
		    return can_move = true;
	    }
	    
	    else if(move == 25 && board[place - 1].type == " " && board[place -2].type == " " && board[place-3].type == " " && board[place -4].type == "c" && board[place -4].color == color && inCheck != color)
	    {
		    return can_move = true;
	    }
	    else if(p[0].type == "p" && move > 2 && board[place+update].color != color && board[place+update].type != " ")
	    {
		    return can_move = true;
	    }
	    else if(board[place+update].color != color)
	    {
	        can_move = true;
	    }
	    return can_move;
    ...
    static bool off_board(int next, int previous, int move)
    {
	    ...
	    {
		    if(board[next].column < board[previous].column){return true;}//make a consolewrite line here to figure out what the columns and stuff are.
	    }
    ...
    static bool get_num_moves(List<peice>p, int selectMove, string color)//get the number of times the player want to move said peice.
    {
		  ...
		  while(!avalible)
		  {
		    ...
		    if(p[0].type == "p" && selectMove == 1 && selectNumber >1 && (p[0].row == 7 || p[0].row == 2))
		    {
			    selectNumber = 2;
		    } 

		    else if(selectMove == 25 || selectMove ==26)
		    {
			    selectNumber = 2;
		    }
    ...
    static bool finialize_move(List<peice> p, int selectMove, int selectNumber, string color)
    {
	    ...
	    if(selectMove == 25)
	    {
		    new_row = board[p1+update+1].row;
		    new_column = board[p1+update+1].column;
		    board[p1] = board[p1+update+1];
		    board[p1].column = old_column;
		    board[p1].row = old_row;
		    board[p1+update+1] = create_empty(0," ",new_row, new_column);
	    }

	    else if(selectMove == 26)
	    {
		    new_row = board[p1+update-1].row;
		    new_column = board[p1+update-1].column;
		    board[p1] = board[p1+update-1];
